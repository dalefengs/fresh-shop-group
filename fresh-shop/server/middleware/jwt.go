...
func JWTAuth() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if token == "" {
			response.FailWithTookenExpired(gin.H{"reload": true}, "未登录或非法访问", c)
			c.Abort()
			return
		}
		if jwtService.IsBlacklist(token) {
			response.FailWithTookenExpired(gin.H{"reload": true}, "您的帐户异地登陆或令牌失效", c)
			c.Abort()
			return
		}
		j := utils.NewJWT()
		...
		if err != nil {
			if err == utils.TokenExpired {
				response.FailWithTookenExpired(gin.H{"reload": true}, "授权已过期", c)
				c.Abort()
				return
			}
