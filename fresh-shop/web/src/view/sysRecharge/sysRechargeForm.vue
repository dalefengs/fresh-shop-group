    ...
    <div class="gva-form-box">
      <el-form :model="formData" ref="elFormRef" label-position="right" :rules="rule" label-width="80px">
        ...
        <el-form-item label="管理员:" prop="adminName">
          <el-input v-model="formData.adminName" :clearable="true" placeholder="请输入" />
        </el-form-item>
...
const init = async () => {
 // 建议通过url传参获取目标数据ID 调用 find方法进行查询数据操作 从而决定本页面是create还是update 以下为id作为url参数示例
    if (route.query.id) {
      const res = await findSysRecharge({ ID: route.query.id })
      if (res.code === 0) {
        formData.value = res.data.resysRecharge
        type.value = 'update'
      }
    } else {
      type.value = 'create'
...
const save = async() => {
      elFormRef.value?.validate( async (valid) => {
         if (!valid) return
           ...
           switch (type.value) {
             case 'create':
               res = await createSysRecharge(formData.value)
               break
             case 'update':
               res = await updateSysRecharge(formData.value)
               break
             default:
               res = await createSysRecharge(formData.value)
               break
