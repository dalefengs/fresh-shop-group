<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="收货人姓名" prop="name" width="120" />
        <el-table-column align="left" label="手机号" prop="mobile" width="120" />
        <el-table-column align="left" label="收货人地区编码" prop="area" width="120" />
        ...
        <el-table-column align="left" label="操作">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateUserAddressFunc(scope.row)">变更</el-button>
...
const getTableData = async() => {
  const table = await getUserAddressList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteUserAddressByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateUserAddressFunc = async(row) => {
    const res = await findUserAddress({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteUserAddressFunc = async (row) => {
    const res = await deleteUserAddress({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
